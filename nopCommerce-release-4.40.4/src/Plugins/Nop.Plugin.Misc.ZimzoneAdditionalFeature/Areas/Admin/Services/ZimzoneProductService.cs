...
{
    public class ZimzoneProductService : IZimzoneProductService
    {
        #region Fields
        ...
        #region Ctor
        public ZimzoneProductService(CatalogSettings catalogSettings,
            ...
            int stockOptionId = 0)
        {
            ...
            // filter with stock options
            if (stockOptionId == 1)
            {
                productsQuery = productsQuery.Where(x => x.StockQuantity > 0 || x.ManageInventoryMethodId == (int)ManageInventoryMethod.DontManageStock);
            }
            else if (stockOptionId == 2)
            {
                productsQuery = productsQuery.Where(x => x.StockQuantity <= 0 && x.ManageInventoryMethodId == (int)ManageInventoryMethod.ManageStock);
            }
